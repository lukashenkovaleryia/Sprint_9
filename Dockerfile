...
# версия интерпретатора указывается проектная - как в PyCharm
FROM python:3.9
...
# в текущую директорию контейнера (это /app).
...
# Скопировать проект, т.е. всё необходимое содержимое 
# той директории локального компьютера, где сохранён Dockerfile,
# в текущую рабочую директорию образа в контейнере — /app.
COPY . .

# При старте контейнера запустить тесты
